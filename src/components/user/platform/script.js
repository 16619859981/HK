...
export default {
  ...
  methods: {
    ...
    handleList2() {
      ...
      if (this.from.personStatus == 2) {
        this.from.personStatus = ''
      }
    ...
    // 渲染列表
    handleList() {
      this.$http.post(api.getPlatform('/person/allPerson'), this.from).then(res => {
        if (res.data.status === 200) {
          this.loading = false
          this.total = Number(res.data.data.total)
          for (var i = 0; i < res.data.data.data.length; i++) {
            if (res.data.data.data[i].personStatus == "1") {
              res.data.data.data[i].personStatus = "启用"
            } else {
              res.data.data.data[i].personStatus = "禁用"
            }
